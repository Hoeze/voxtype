...
Voxtype uses a fallback chain: portal → wtype → eitype → dotool → ydotool → clipboard (wl-copy) → xclip (portal is first with the default `portal` feature; non-portal builds start at wtype)
