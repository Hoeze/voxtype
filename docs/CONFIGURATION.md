...
When config.toml has an `[openvino]` section with `device = "NPU"`, `voxtype setup` runs this compile itself right after downloading or activating a model, instead of leaving the wait to your first recording. It is skipped when the model's cache blob already exists. A failed setup-time compile prints a warning and setup continues: the model files and config changes are still in place, and the daemon compiles the model on first use, falling back to GPU or CPU when the NPU is unavailable. See the troubleshooting guide entry on the setup-time NPU compile for details.
...
**Type:** Array of strings
**Default:** `["portal", "wtype", "eitype", "dotool", "ydotool", "clipboard", "xclip"]` (portal is present when built with the default `portal` feature; otherwise the chain starts at `wtype`)
**Required:** No
...
- `portal` - persistent XDG RemoteDesktop portal session (KDE Plasma 6, GNOME, wlroots). Like `eitype` it needs no uinput or `input` group, but it holds ONE portal session for the life of the daemon instead of re-registering per call, so it avoids the eitype tray flicker and per-call latency during streaming dictation on KDE. Layout-independent: it sends Unicode keysyms directly and ignores the `*_xkb_layout`/`*_xkb_variant` hints. Compiled in and enabled by default (the `portal` cargo feature) and **tried first** in the default chain. First use shows a one-time consent dialog; the grant persists via a restore token cached under `$XDG_CACHE_HOME/voxtype/`. To exclude it, set an explicit `driver_order` without `portal`, or build with `--no-default-features`.
...
The default chain is: portal → wtype → eitype → dotool → ydotool → clipboard → xclip (portal is first with the default `portal` feature; non-portal builds start at wtype)
